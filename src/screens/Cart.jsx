...
import { View, Text, SafeAreaView, StyleSheet, FlatList, TouchableOpacity, Image, Alert } from 'react-native';
import { Header, width } from '../utilites/helper/Helper';
import { useSelector, useDispatch } from 'react-redux';
import { addProductToMyCart, removeProductFromCart, clearCart, deleteProductFromCart, loadCartFromStorage } from '../../Redux/MyCartSlice';
    ...
    <SafeAreaView style={Styles.main}>
        ...
        <View>
          {myCart.length > 0 && (
            <TouchableOpacity onPress={() => toggleModal()}>
              <Image source={require('../utilites/images/19.png')} style={{ height: 25, width: 50 }} resizeMode="contain" />
            </TouchableOpacity>
          )}
        </View>
      </View>
          ...
          renderItem={({ item }) => (
            <View style={Styles.touch_detail}>
              <View style={{ flexDirection: 'row' }}>
                ...
                <View style={{ justifyContent: 'center', marginTop: 15 }}>
                  ...
                  <View style={Styles.qtybtn}>
                    <TouchableOpacity style={Styles.minus} onPress={() => dispatch(removeProductFromCart(item))}>
                      <Text style={Styles.carttxt}>-</Text>
                    </TouchableOpacity>

                    <Text>{item.qty}</Text>

                    <TouchableOpacity style={Styles.plus} onPress={() => dispatch(addProductToMyCart(item))}>
                      <Text style={Styles.carttxt}>+</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              ...
              <View style={{ position: 'absolute', right: 5, marginVertical: 5 }}>
                <TouchableOpacity style={Styles.minus} onPress={() => dispatch(deleteProductFromCart(item))}>
      ...
      <Modal isVisible={isModalVisible}>
        <View style={Styles.modal}>
          ...
          <View style={{ flexDirection: 'row', marginTop: 40, justifyContent: 'space-between', width: width / 1.5, alignSelf: 'center' }}>
            <TouchableOpacity onPress={toggleModal} style={Styles.yesno}>
              <Text style={{ fontSize: 19, fontWeight: '500', color: 'white' }}>No</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => yesModal()} style={Styles.yesno}>
              <Text style={{ fontSize: 19, fontWeight: '500', color: 'white' }}>Yes</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
